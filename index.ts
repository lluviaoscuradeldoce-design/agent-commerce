...
const plugin = {
    ...
    register(api: OpenClawPluginApi) {
        ...
        const stateDir = api.runtime.paths?.stateDir
            ?? (process.env.OPENCLAW_STATE_DIR || `${process.env.HOME ?? process.env.USERPROFILE}/.openclaw`);
        ...
        const escrow = contractAddress
            ? new EscrowManager(stateDir, contractAddress)
            : null;
